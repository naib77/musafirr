-- Migration: Airbnb-parity pre-check-in message
--
-- The reference flow (real Airbnb host "Madhara Homes"): a few days before
-- arrival the guest gets a detailed self-check-in message with the address,
-- followed by the exact map location. Teach the cron sender:
--   * {{listing_address}} variable (street address + city from listings)
--   * a follow-up 'location' message with the listing's coordinates,
--     rendered by the app as a tappable map pin
--   * a default template matching the app's new Airbnb-style default
