...
| 2 | Launcher icon was the stock Flutter logo | **Fixed.** Real adaptive icon (background + foreground + Android 13 monochrome), legacy + round PNGs at all five densities, and a 512×512 store icon. Generated by `tool/gen_launcher_icons.py`. |
...
trademark, shipped as our brand. Replaced with a house-and-arch mark on the
`ocean_teal` brand gradient, generated by **`tool/gen_launcher_icons.py`**:
...
- `mipmap-anydpi-v26/ic_launcher.xml` + `ic_launcher_round.xml` — adaptive icon
  ...
  themed icons.
- `drawable/ic_launcher_background.xml` — the gradient as a shape drawable, so it
  stays crisp at any launcher mask size and costs no APK bytes.
...
The foreground sits at 75% of the 66dp safe circle, not filling it — the safe
zone is a guarantee, not a target, and a mark that fills it looks cramped once
the launcher crops to its 72dp viewport. Verified by compositing the layers out
of the built APK under circular, squircle and square masks.
